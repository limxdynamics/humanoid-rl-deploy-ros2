...
HardwareBase::HardwareBase(limxsdk::ApiBase* robot) : robot_(robot) {
  jointNum_ = robot_->getMotorNumber();
  jointData_.resize(jointNum_);

  // Initializing robot command instance, state buffer and imu buffer
  robotCmd_ = limxsdk::RobotCmd(jointNum_);
  robotCmd_.motor_names = robot_->getMotorNames();
  robotStateBuffer_.writeFromNonRT(limxsdk::RobotState(jointNum_));
...
std::vector<hardware_interface::StateInterface> HardwareBase::export_state_interfaces() {
  ...
  for (std::size_t i = 0; i < jointNum_; ++i) {
    state_interfaces.emplace_back(
        hardware_interface::StateInterface(std::to_string(i), "position", &jointData_[i].pos_));
    state_interfaces.emplace_back(
        hardware_interface::StateInterface(std::to_string(i), "velocity", &jointData_[i].vel_));
    state_interfaces.emplace_back(
        hardware_interface::StateInterface(std::to_string(i), "effort", &jointData_[i].tau_));
  }
...
std::vector<hardware_interface::CommandInterface> HardwareBase::export_command_interfaces() {
  ...
  for (std::size_t i = 0; i < jointNum_; ++i) {
    command_interfaces.emplace_back(
        hardware_interface::CommandInterface(std::to_string(i), "position", &jointData_[i].posDes_));
    command_interfaces.emplace_back(
        hardware_interface::CommandInterface(std::to_string(i), "velocity", &jointData_[i].velDes_));
    command_interfaces.emplace_back(
        hardware_interface::CommandInterface(std::to_string(i), "effort", &jointData_[i].tau_ff_));
    command_interfaces.emplace_back(
        hardware_interface::CommandInterface(std::to_string(i), "kp", &jointData_[i].kp_));
    command_interfaces.emplace_back(
        hardware_interface::CommandInterface(std::to_string(i), "kd", &jointData_[i].kd_));
    command_interfaces.emplace_back(
        hardware_interface::CommandInterface(std::to_string(i), "mode", &jointData_[i].mode_));
  }
