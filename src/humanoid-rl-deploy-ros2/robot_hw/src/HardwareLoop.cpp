  ...
  try {
    ...
    for (const auto &controller : controllers) {
      ...
      if (pos != std::string::npos) {
        std::string controller_name = controller.substr(pos + 1);
...
/**
 * @brief Switch a specified controller.
 * 
 ...
 */
bool HardwareLoop::switchController(const std::string &controller_name) {
...
bool HardwareLoop::startController(const std::string &controller_name) {
  ...
  std::vector<std::string> start_controllers = {controller_name};
  std::vector<std::string> stop_controllers = {};
  RCLCPP_INFO(rclcpp::get_logger("HardwareLoop"), "Starting controllers...");
  controller_manager_->switch_controller(
      start_controllers,
      stop_controllers,
      controller_manager_msgs::srv::SwitchController::Request::BEST_EFFORT,
